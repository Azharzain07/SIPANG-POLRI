<x-app-layout>
                ...
                <div class="p-6 md:p-8 text-gray-900">
                    <form method="POST" action="{{ route('pengajuan.update', $pengajuan) }}">
                        ...
                        <div class="space-y-6">
                            ...
                            <fieldset id="informasi-dasar" class="border p-4 rounded-md">
                                ...
                                <div class="space-y-4 pt-4">
                                    <div>
                                        ...
                                        <input id="tanggal_pengajuan" type="date" name="tanggal_pengajuan" class="form-input mt-1 block w-full rounded-md shadow-sm border-gray-300" required value="{{ old('tanggal_pengajuan', $pengajuan->tanggal_pengajuan->format('Y-m-d')) }}">
                                    </div>
                                    <div>
                                        <label for="sumber_dana" class="block font-medium text-sm text-gray-700">Sumber Dana</label>
                                        <select id="sumber_dana" name="sumber_dana_id" class="form-select mt-1 block w-full rounded-md shadow-sm border-gray-300" required>
                                            <option value="">-- Pilih --</option>
                                            @foreach ($sumberDanas as $sumber)
                                                <option value="{{ $sumber->id }}" {{ old('sumber_dana_id', $pengajuan->sumber_dana_id) == $sumber->id ? 'selected' : '' }}>
                                    ...
                                    <div>
                                        ...
                                        <textarea id="uraian" name="uraian" class="form-textarea mt-1 block w-full rounded-md shadow-sm border-gray-300" rows="3" required>{{ old('uraian', $pengajuan->uraian) }}</textarea>
                                    </div>
                                </div>
                            </fieldset>

                            <fieldset id="detail-kegiatan" class="border p-4 rounded-md">
                                ...
                                <div class="space-y-4 pt-4">
                                    <div>
                                        ...
                                        <select id="program" name="program_id" class="form-select mt-1 block w-full rounded-md shadow-sm border-gray-300" required>
                                            ...
                                            @foreach ($programs as $program)
                                                <option value="{{ $program->id }}" {{ old('program_id', $pengajuan->program_id) == $program->id ? 'selected' : '' }}>
                                    ...
                                    <div>
                                        ...
                                        <select id="aktivitas" name="activity_id" class="form-select mt-1 block w-full rounded-md shadow-sm border-gray-300" required>
                                            <option value="">-- Pilih Program Dulu --</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label for="kro" class="block font-medium text-sm text-gray-700">KRO</label>
                                        <select id="kro" name="kro_id" class="form-select mt-1 block w-full rounded-md shadow-sm border-gray-300" required>
                            ...
                            <fieldset id="rincian-anggaran" class="border p-4 rounded-md mt-6">
                                        ...
                                        <table class="min-w-full divide-y divide-gray-200">
                                            ...
                                            <tbody id="rincian-body">
                                                @foreach ($pengajuan->details as $detail)
                                                    <tr>
                                                        <td class="px-4 py-2 border-b">{{ $detail->coa->account->nama_akun_belanja }}</td>
                                                        <td class="px-4 py-2 border-b">{{ $detail->coa->nama_coa }}</td>
                                                        <td class="px-4 py-2 text-right border-b">{{ 'Rp ' . number_format($detail->jumlah_diajukan, 0, ',', '.') }}</td>
                                                        <td class="px-4 py-2 text-center border-b">
                                                            <button type="button" class="text-red-500 hover:text-red-700 remove-item-btn">Hapus</button>
                                                        </td>
                                                        <input type="hidden" name="details[{{ $detail->coa_id }}][coa_id]" value="{{ $detail->coa_id }}">
                                                        <input type="hidden" name="details[{{ $detail->coa_id }}][jumlah_diajukan]" value="{{ $detail->jumlah_diajukan }}">
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
                             ...
                             <div class="flex justify-end pt-4">
                                <button type="submit" id="submit-button" class="px-6 py-2 bg-gray-800 text-white rounded-md">
                                    Update Pengajuan
                                </button>
                            </div>
    ...
    @push('scripts')
    <script>
    // Menyimpan ID awal dari data yang diedit untuk inisialisasi
    const initialData = {
        programId: {{ old('program_id', $pengajuan->program_id) ?? 'null' }},
        activityId: {{ old('activity_id', $pengajuan->activity_id) ?? 'null' }},
        kroId: {{ old('kro_id', $pengajuan->kro_id) ?? 'null' }}
    };
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // --- Fungsi Helper ---
        function populateDropdown(selectElement, data, defaultOption, valueKey, textKey, selectedValue = null) {
        ...
        // --- Fungsi Fetch Data Dinamis ---
        function fetchActivities(programId, selectedActivityId = null) {
            ...
            fetch(`{{ url('/get-activities') }}/${programId}`)
                ...
                .then(data => {
                    populateDropdown(elements.aktivitas, data, 'Pilih Aktivitas', 'id', 'nama_aktivitas', selectedActivityId);
                    elements.aktivitas.disabled = false;
                    if (selectedActivityId) {
                        elements.aktivitas.dispatchEvent(new Event('change'));
                    }
                })
                .catch(error => {
        ...
        function fetchKros(activityId, selectedKroId = null) {
            ...
            fetch(`{{ url('/get-kros') }}/${activityId}`)
                .then(response => response.json())
                .then(data => {
                    populateDropdown(elements.kro, data, 'Pilih KRO', 'id', 'nama_kro', selectedKroId);
                    elements.kro.disabled = false;
                    if (selectedKroId) {
                        elements.kro.dispatchEvent(new Event('change'));
                    }
                })
                .catch(error => {
        ...
        if (initialData.programId) {
            fetchActivities(initialData.programId, initialData.activityId);
        }
